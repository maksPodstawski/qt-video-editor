      ...
      currentStateIndex(-1)
{
    ...
    connect(indicator, &Indicator::requestCurrentVideo, this, [this]()
    {
        ...
        {
            qDebug() << "Video:" << video.getTitle() << "Start:" << video.getStartTime() << "End:" << video.
                getEndTime();
        }
    });
...
QTime TimeLine::getVideoDurationTime() const
{
...
int TimeLine::getVideoDurationWidth() const
{
    int totalWidth = 0;
    for (const VideoData& video : videoList)
    {
...
int TimeLine::getTotalVideosDurationTime() const
{
    int time = 0;
    ...
    if (!videoList.isEmpty())
    {
        ...
        {
            if (videoList[i].getEndTime() == -1)
            {
                int durationInMilliseconds = QTime::fromString(videoList[i].getDuration(), "mm:ss").
                    msecsSinceStartOfDay();
                time += durationInMilliseconds - videoList[i].getStartTime();
            }
            else
            {
...
qreal TimeLine::getTimePerUnit() const
{
...
void TimeLine::splitVideoAtIndicator()
{
    ...
    if (currentVideo)
    {
        ...
        VideoData newVideo = VideoData(currentVideo->getTitle(), currentVideo->getDuration(),
                                       currentVideo->getFilePath(), currentVideo->getExtension(), newRect2);
